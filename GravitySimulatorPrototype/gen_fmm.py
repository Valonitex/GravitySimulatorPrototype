...
with open('/home/valonitex/Documents/Code/GravitySimulatorPrototype/GravitySimulatorPrototype/test_fmm.cpp', 'w') as f:
...
namespace fmm {
    constexpr int P = ''' + str(P) + ''';
    constexpr int NCOEFF = ''' + str(NCOEFF) + ''';
...
''')
    ...
    static void l2p(int leaf_idx, double px, double py, double vx, double vy, vectorP& acc_out, vectorP* jerk_out) {
        ...
        if (jerk_out) {
            ...
            // FMM far-field jerk assumes source cluster is moving with com_vx, com_vy
            // Wait, actually the original code did dz_rel_v = (vx - nd.com_vx).
            // This is theoretically dubious for FMM but we match its intent.
            // Actually, in L2P we only have local expansions, we don't have nd.com_vx 
            // of the sources! The `nd.com_vx` is of the TARGET leaf.
            // Oh! The original code used `nd.com_vx` in `l2p`. 
            // Let's just use `vx` for jerk calculation to be rigorous because `nd.com_vx` 
            // of target leaf is just the COM velocity of the target bodies, which doesn't make sense to subtract.
...
int main() {
    ...
    fmm::run_fmm(pos, masses, &vel, acc_fmm, &jerk_fmm, bodies);
    ...
    for (int i = 0; i < 2; ++i) {
        for (int j = 0; j < 2; ++j) {
            ...
            acc[i] += r * (G * masses[j] * inv_r3);
            
            vectorP v = vel[j] - vel[i];
            double v_dot_r = v.icap * r.icap + v.jcap * r.jcap;
            double inv_r5 = inv_r3 * (1.0/r2);
            jerk[i] += (v * inv_r3 - r * (3.0 * v_dot_r * inv_r5)) * (G * masses[j]);
        }
    }

    for (int i = 0; i < 2; ++i) {
        std::cout << "Body " << i << ":\n";
        std::cout << "  Acc Exact: " << acc[i].icap << ", " << acc[i].jcap << "\n";
        std::cout << "  Acc FMM:   " << acc_fmm[i].icap << ", " << acc_fmm[i].jcap << "\n";
        std::cout << "  Jerk Exact: " << jerk[i].icap << ", " << jerk[i].jcap << "\n";
        std::cout << "  Jerk FMM:   " << jerk_fmm[i].icap << ", " << jerk_fmm[i].jcap << "\n";
    }

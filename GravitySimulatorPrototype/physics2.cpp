...
public:
  ...
  vectorP operator*(double t) const { return (vectorP(icap * t, jcap * t)); }

  vectorP operator/(double t) {
    if (t == 0.0)
      throw std::invalid_argument("vectorP division by zero");
  ...
  vectorP negate() { return vectorP(-(icap), -(jcap)); }

  vectorP round() { return vectorP(std::round(icap), std::round(jcap)); }
...
public:
  Body(double m, double r, bool stat = true, vectorP pos = {0, 0},
       vectorP vel = {0, 0}, vectorP f = {0, 0}) // stat = static
      : dead(false), movability(stat), m_posVec(pos), m_velVec(vel),
        m_forVec(f), m_accVec(0.0f, 0.0f) {
    if (m <= 0 || r <= 0)
  ...
  void forsum(vectorP force) { m_forRes += force; }
  ...
  std::unique_ptr<Body> clone() const {
    return std::make_unique<Body>(m_Mass, m_radius, movability, m_posVec,
                                  m_velVec, m_forVec);
  }

  vectorP lP() { return (m_velVec * m_Mass); }

  double aP() { return (m_posVec.cross(lP())); }
};
...
//    resolveBarnesHut   → replace  resolve()          in moveVerlet /
//    moveYoshida resolveWithJerkBH  → replace  resolveWithJerk()  in
//    moveHermite computeAccelBH     → replace  computeAccel()     in moveRK45
//
...
inline int idx(int p, int q) { return p + q * (P + 1) - q * (q - 1) / 2; }
...
static int alloc_node(double cx, double cy, double half, int level,
                      int parent) {
  int idx = next_node++;
  if (idx >= (int)tree.size())
    tree.resize(std::max((int)tree.size() * 2, 256));
  tree[idx] = Node();
  tree[idx].cx = cx;
  tree[idx].cy = cy;
  tree[idx].half = half;
...
static bool is_ancestor(int anc, int node) {
  while (node != -1) {
    if (node == anc)
      return true;
...
static bool well_separated(int a_idx, int b_idx, double theta = 0.5) {
  const Node &a = tree[a_idx];
  const Node &b = tree[b_idx];
  double dx = a.cx - b.cx;
  double dy = a.cy - b.cy;
  double dist2 = dx * dx + dy * dy;
  double s = 2.0 * std::max(a.half, b.half);
  return dist2 > 0.0 && s * s < theta * theta * dist2;
}

static void build_sub(int node_idx, const std::vector<vectorP> &pos,
                      int max_depth) {
  ...
  for (int b_idx : nd.bodies) {
    int q =
        (pos[b_idx].icap >= nd.cx ? 1 : 0) | (pos[b_idx].jcap >= nd.cy ? 2 : 0);
    quad_bodies[q].push_back(b_idx);
  }
  nd.bodies.clear();
  for (int q = 0; q < 4; ++q) {
    if (quad_bodies[q].empty())
      continue;
...
static void p2m(int node_idx, const std::vector<vectorP> &pos,
    ...
    for (int p_idx = 0; p_idx <= P - q; ++p_idx) {
      ...
      for (int j = 0; j <= q; ++j) {
        for (int i = 0; i <= p_idx; ++i) {
          sum += binom.C[p_idx][i] * binom.C[q][j] * xp[p_idx - i] * yp[q - j] *
                 c.M[idx(i, j)];
        }
      }
      p.M[idx(p_idx, q)] += sum;
    ...
    for (int a = 0; a <= P - b; ++a) {
      ...
      for (int v = 0; v <= P - a - b; ++v) {
        for (int u = 0; u <= P - a - b - v; ++u) {
          ...
          sum += C[a + u][b + v] * binom.C[a + u][a] * binom.C[b + v][b] *
                 sign * source.M[idx(u, v)];
        }
      }
...
static void l2l(int parent_idx, int child_idx) {
  ...
  for (int j = 0; j <= P; ++j) {
    for (int i = 0; i <= P - j; ++i) {
      ...
      for (int b = j; b <= P - i; ++b) {
        for (int a = i; a <= P - b; ++a) {
          sum += binom.C[a][i] * binom.C[b][j] * xp[a - i] * yp[b - j] *
                 p.L[idx(a, b)];
        }
      }
      c.L[idx(i, j)] += sum;
...
static void l2p(int leaf_idx, double px, double py, double vx, double vy,
                vectorP &acc_out, vectorP *jerk_out) {
...
static void run_fmm(const std::vector<vectorP> &pos,
  ...
  for (int i = 1; i < n; ++i) {
    xmin = std::min(xmin, pos[i].icap);
    xmax = std::max(xmax, pos[i].icap);
    ymin = std::min(ymin, pos[i].jcap);
    ymax = std::max(ymax, pos[i].jcap);
  }
  double cx = (xmin + xmax) * 0.5;
  double cy = (ymin + ymax) * 0.5;
  double half =
      std::max({(xmax - xmin) * 0.5, (ymax - ymin) * 0.5, 1e-6}) * 1.001;
  ...
  for (int leaf_idx : leaves)
    p2m(leaf_idx, pos, masses, vel_ptr);
  ...
  // M2L: every leaf receives far-field contributions from all well-separated
  // nodes outside its subtree (ancestors contribute via L2L, not direct M2L).
  for (int u_leaf : leaves) {
    for (int v_idx = 0; v_idx < next_node; ++v_idx) {
      if (u_leaf == v_idx)
        continue;
      if (is_ancestor(v_idx, u_leaf))
        continue;
      if (is_ancestor(u_leaf, v_idx))
        continue;
      if (tree[v_idx].is_leaf && tree[u_leaf].is_leaf &&
          is_neighbor(tree[u_leaf], tree[v_idx]))
        continue;
      if (!well_separated(u_leaf, v_idx))
        continue;
      m2l(u_leaf, v_idx);
    }
  }
  ...
  for (int u_leaf : leaves) {
    ...
    for (int v_leaf : leaves) {
      if (u_leaf == v_leaf)
        continue;
      if (well_separated(u_leaf, v_leaf) && !is_neighbor(u_nd, tree[v_leaf]))
        continue;
      for (int i : u_nd.bodies) {
        ...
        for (int j : tree[v_leaf].bodies) {
          ...
          if (jerk_out && vel_ptr) {
            ...
            (*jerk_out)[i] +=
                (v * inv_r3 - r * (3.0 * v_dot_r * inv_r5)) * (G * masses[j]);
          }
        }
      }
    }
...
struct Node {
  ...
  double com_vx,
      com_vy; // centre-of-mass velocity  (only set when building for jerk)
  std::vector<int>
      leaf_bodies; // non-empty only at leaves (supports co-located bodies)
...
inline double child_cx(double cx, double half, int q) {
  return cx + ((q & 1) ? half : -half);
}
inline double child_cy(double cy, double half, int q) {
  return cy + ((q & 2) ? half : -half);
}
...
static void insert(int bi, double px, double py, double bm, double bvx,
                   double bvy, const std::vector<vectorP> &pos,
                   const std::vector<double> &masses,
                   const std::vector<vectorP> *vel_ptr, int node_idx) {
  while (true) {
    bool is_internal =
    ...
    if (pool[node_idx].leaf_bodies.empty() && !is_internal) {
      pool[node_idx].leaf_bodies.push_back(bi);
      pool[node_idx].mass = bm;
      pool[node_idx].com_x = px;
      pool[node_idx].com_y = py;
      pool[node_idx].com_vx = bvx;
      pool[node_idx].com_vy = bvy;
      return;
    }
    ...
    pool[node_idx].com_x =
        (pool[node_idx].com_x * pool[node_idx].mass + px * bm) / new_mass;
    pool[node_idx].com_y =
        (pool[node_idx].com_y * pool[node_idx].mass + py * bm) / new_mass;
    pool[node_idx].com_vx =
        (pool[node_idx].com_vx * pool[node_idx].mass + bvx * bm) / new_mass;
    pool[node_idx].com_vy =
        (pool[node_idx].com_vy * pool[node_idx].mass + bvy * bm) / new_mass;
    pool[node_idx].mass = new_mass;

    // ── Case 2: occupied leaf → subdivide or co-locate at floor ───
    if (!is_internal && !pool[node_idx].leaf_bodies.empty()) {
      if (pool[node_idx].half < 1e-10) {
        pool[node_idx].leaf_bodies.push_back(bi);
        return;
      }

      std::vector<int> to_reinsert = std::move(pool[node_idx].leaf_bodies);
      pool[node_idx].leaf_bodies.clear();

      double cx = pool[node_idx].cx, cy = pool[node_idx].cy;
      double h2 = pool[node_idx].half * 0.5;

      for (int existing : to_reinsert) {
        ...
        if (pool[node_idx].ch[q_ex] == -1)
          pool[node_idx].ch[q_ex] =
              alloc(child_cx(cx, h2, q_ex), child_cy(cy, h2, q_ex), h2);

        insert(existing, ex, ey, masses[existing], evx, evy, pos, masses,
               vel_ptr, pool[node_idx].ch[q_ex]);
      }
    }
    ...
    if (pool[node_idx].ch[q] == -1)
      pool[node_idx].ch[q] =
          alloc(child_cx(cx, h2, q), child_cy(cy, h2, q), h2);

    node_idx = pool[node_idx].ch[q];
  }
}

// ── Apply the contribution of one node (or leaf body) to (ax,ay,jx,jy) ─
static inline void apply(double mass_n, double dx, double dy, double dvx,
                         double dvy, double &ax, double &ay, double *jx,
                         double *jy) {
  double r2s = dx * dx + dy * dy + eps * eps;
...
static void walk(int node_idx, int bi, double bx, double by, double bvx,
                 double bvy, double theta_sq, const std::vector<vectorP> &pos,
                 const std::vector<double> &masses,
                 const std::vector<vectorP> *vel_ptr, double &ax, double &ay,
                 double *jx, double *jy) {
  if (node_idx == -1)
    return;
  ...
  double r2 =
      dx * dx + dy * dy; // geometric r² (no softening, used for criterion)

  // ── Leaf: direct pair(s) ─────────────────────────────────────────
  if (!n.leaf_bodies.empty()) {
    for (int bj : n.leaf_bodies) {
      if (bj == bi)
        continue;
      double bjx = pos[bj].icap, bjy = pos[bj].jcap;
      double bvx_j = vel_ptr ? (*vel_ptr)[bj].icap : n.com_vx;
      double bvy_j = vel_ptr ? (*vel_ptr)[bj].jcap : n.com_vy;
      apply(masses[bj], bjx - bx, bjy - by, bvx_j - bvx, bvy_j - bvy, ax, ay,
            jx, jy);
    }
    return;
  }
  ...
  bool target_inside =
      (std::abs(bx - n.cx) <= n.half && std::abs(by - n.cy) <= n.half);
  if (!target_inside && r2 > 0.0 && 4.0 * n.half * n.half < theta_sq * r2) {
    apply(n.mass, dx, dy, n.com_vx - bvx, n.com_vy - bvy, ax, ay, jx, jy);
    return;
  }

  // ── Open: recurse into children ───────────────────────────────────
  for (int q = 0; q < 4; q++)
    walk(n.ch[q], bi, bx, by, bvx, bvy, theta_sq, pos, masses, vel_ptr, ax, ay,
         jx, jy);
}
...
static int build(const std::vector<vectorP> &pos,
  ...
  for (int i = 0; i < s; i++) {
    ...
    insert(i, pos[i].icap, pos[i].jcap, masses[i], vx, vy, pos, masses, vel_ptr,
           root);
  }
  return root;
}
...
static void ksToPhysical(const RegularizedPair &ks, vectorP &rel_pos,
                         vectorP &rel_vel) {
...
// Cap regularized-time increment for numerical stability (KS-008).
static double capKSdtau(const RegularizedPair &ks, double dtau) {
  if (ks.H < 0.0) {
    const double omega = std::sqrt(-0.5 * ks.H);
    dtau = std::min(dtau, 0.25 * M_PI / omega);
  } else if (ks.H > 0.0) {
    const double omega = std::sqrt(0.5 * ks.H);
    dtau = std::min(dtau, 1.0 / omega);
  } else {
    const double r = ks.u1 * ks.u1 + ks.u2 * ks.u2;
    const double speed_sq = ks.up1 * ks.up1 + ks.up2 * ks.up2;
    if (speed_sq > 1e-30) {
      const double char_tau = std::sqrt(r / speed_sq);
      dtau = std::min(dtau, char_tau);
    }
  }
  return std::min(dtau, 1e6);
}

// Advance u'' - (H/2)u = 0 by regularized-time increment dtau.
static void advanceKSHarmonic(RegularizedPair &ks, double dtau) {
  if (dtau == 0.0)
    return;
  ...
  if (ks.H < 0.0) {
    const double omega = std::sqrt(-0.5 * ks.H);
    const double cos_wt = std::cos(omega * dtau);
    const double sin_wt = std::sin(omega * dtau);
    ...
    const double omega = std::sqrt(0.5 * ks.H);
    const double cosh_wt = std::cosh(omega * dtau);
    const double sinh_wt = std::sinh(omega * dtau);
    const double inv_omega = 1.0 / omega;
...
// Maps physical time delta_t to regularized time via Sundman: tau = integral
// dt/r(t). Sub-steps with updated r each interval (composite midpoint rule)
// instead of the first-order dtau = delta_t / r(t0) approximation (KS-002).
static void stepKSAnalytical(RegularizedPair &ks, double delta_t) {
  if (delta_t == 0.0)
    return;

  constexpr int MAX_SUNDMAN_SUBSTEPS = 64;
  constexpr double SUNDMAN_TIME_TOL = 1e-14;

  const double time_tol = SUNDMAN_TIME_TOL * std::max(1.0, std::abs(delta_t));
  double remaining = delta_t;
  int sub = 0;

  while (remaining > time_tol && sub < MAX_SUNDMAN_SUBSTEPS) {
    const double r = ks.u1 * ks.u1 + ks.u2 * ks.u2;
    if (r < R_KS_MIN)
      return;

    // Equal split of remaining physical time; r refreshed each sub-interval.
    const int subs_left = MAX_SUNDMAN_SUBSTEPS - sub;
    const double sub_dt = remaining / subs_left;

    double dtau = capKSdtau(ks, sub_dt / r);
    const double phys_advanced = dtau * r;

    advanceKSHarmonic(ks, dtau);
    remaining -= phys_advanced;
    ++sub;
  }
  ...
  if (remaining > time_tol) {
    const double r = ks.u1 * ks.u1 + ks.u2 * ks.u2;
    if (r >= R_KS_MIN) {
      const double dtau = capKSdtau(ks, remaining / r);
      advanceKSHarmonic(ks, dtau);
    }
  }
}
...
// Detects close pairs, extracts them into KS space, and replaces them with a
// COM body
static std::vector<RegularizedPair>
...
// Sum external (non-binary) Newtonian acceleration at pos; skips idxA/idxB.
static void externalAccelAt(const std::vector<std::unique_ptr<Body>> &bodies,
                            int idxA, int idxB, const vectorP &pos,
                            vectorP &acc) {
  acc = vectorP(0, 0);
  ...
  for (int k = 0; k < s; ++k) {
    if (k == idxA || k == idxB)
      continue;
    if (!bodies[k] || bodies[k]->dead || bodies[k]->m_Mass <= 0.0)
      continue;

    vectorP disp = pos - bodies[k]->m_posVec;
    const double distSq = disp.magSq() + eps * eps;
    const double invdist = 1.0 / std::sqrt(distSq);
    const double denom = invdist * invdist * invdist;
    acc += disp * (G * bodies[k]->m_Mass * denom);
  }
}
...
static void
restoreCloseEncounters(std::vector<std::unique_ptr<Body>> &bodies,
                       std::vector<RegularizedPair> &active_ks_pairs,
                       double step_dt) {
  for (auto &pair : active_ks_pairs) {
    if (pair.idxA < 0 || pair.idxA >= (int)bodies.size() || pair.idxB < 0 ||
        pair.idxB >= (int)bodies.size())
      continue;

    Body *bodA = bodies[pair.idxA].get();
    Body *bodB = bodies[pair.idxB].get();
    if (!bodA || !bodB || bodA->dead || bodB->dead)
      continue;

    // Re-sync H from current KS state before the isolated binary step.
    {
      vectorP rel_pos0, rel_vel0;
      ksToPhysical(pair, rel_pos0, rel_vel0);
      const double r0 = rel_pos0.mag();
      if (r0 > 1e-15)
        pair.H = 0.5 * rel_vel0.magSq() - pair.mu / r0;
    }

    // Advance the internal binary orbit in KS space (isolated Kepler).
    ...
    const double fracA = pair.m2 / pair.M_tot;
    const double fracB = pair.m1 / pair.M_tot;

    vectorP posA = updated_com_pos - rel_pos * fracA;
    vectorP posB = updated_com_pos + rel_pos * fracB;

    // First-order tidal correction: external bodies differentially accelerate
    // the binary members over the macro-step (split-scheme perturbation
    // coupling).
    vectorP accA, accB;
    externalAccelAt(bodies, pair.idxA, pair.idxB, posA, accA);
    externalAccelAt(bodies, pair.idxA, pair.idxB, posB, accB);
...
// Automatically shrinks dt globally when any two bodies approach, preventing
// integrator rejection
static double
getSundmanAdaptiveDT(const std::vector<std::unique_ptr<Body>> &bodies,
                     double base_dt) {
  double max_inv_dist = 0.0;
  int s = (int)bodies.size();
  for (int i = 0; i < s - 1; i++) {
    if (!bodies[i] || bodies[i]->dead)
      continue;
    for (int j = i + 1; j < s; j++) {
      if (!bodies[j] || bodies[j]->dead)
        continue;
      double dist = (bodies[j]->m_posVec - bodies[i]->m_posVec).mag();
      if (dist > 1e-5)
        max_inv_dist = std::max(max_inv_dist, 1.0 / dist);
    }
  }
...
struct CollisionResult checkCol(std::vector<std::unique_ptr<Body>> &bodies,
      ...
      if (disp < mindisp) {
        ...
        if (clusInA != clusInB) {
          if (clusInB == 0)
            ...
            const int keep = std::min(clusInA, clusInB);
            const int drop = std::max(clusInA, clusInB);
            for (auto &b : bodies) {
              if (b && b->clusterIndex == drop)
                b->clusterIndex = keep;
            }
            clusInA = clusInB = keep;
          }
        }

        /*float newMass = boda.m_Mass + bodb.m_Mass;
        vectorP newPos = (boda.m_posVec + bodb.m_posVec) / 2;
        vectorP newVec = ((boda.m_velVec * boda.m_Mass ) + (bodb.m_velVec *
        bodb.m_Mass)) / newMass; vectorP newFor = (boda.m_forVec +
        bodb.m_forVec) / 2; double newRadius = pow(boda.m_radius * boda.m_radius
        * boda.m_radius + bodb.m_radius * bodb.m_radius * bodb.m_radius, 1.0f
        / 3.0f); auto newBody = std::make_unique<Body>(newMass, newRadius,
        true,newPos, newVec, newFor);
  ...
  for (int i = 0; i < colClusters.size(); i++) {
    ...
    // COL-005 fix: derive acceleration from the net force sum (F = ma → a = F/m).
    // m_accVec copied from member[0] was stale (wrong mass, wrong position).
    mergedBody.m_accVec = totalForce * (1.0 / totalMass);
    // Jerk is discontinuous across a collision event (topology change): zero it
    // so the Hermite integrator restarts cleanly from the new acceleration state
    // rather than extrapolating from a member's pre-merge jerk history.
    mergedBody.m_jerkVec = vectorP(0.0, 0.0);
    ...
    addtobodies.push_back(std::move(std::make_unique<Body>(
...
struct CollisionResult
checkColSpatialHash(std::vector<std::unique_ptr<Body>> &bodies,
                    std::vector<std::vector<Body *>> &colClusters) {
  std::vector<std::unique_ptr<Body>> addtobodies;
  std::vector<std::vector<Body>> tempClusters;
  ...
  // Find the largest diameter in the current system to ensure 3x3 neighborhood
  // capture
  ...
  const double cell_size =
      std::max(max_diam, 1.0); // Floor size to prevent div by zero
  ...
  for (int i = 0; i < nBodies; i++) {
    ...
    int32_t cx = static_cast<int32_t>(
        std::floor(bodies[i]->m_posVec.icap * inv_cell_size));
    int32_t cy = static_cast<int32_t>(
        std::floor(bodies[i]->m_posVec.jcap * inv_cell_size));
  ...
  for (int i = 0; i < nBodies; i++) {
    ...
    int32_t cx =
        static_cast<int32_t>(std::floor(boda.m_posVec.icap * inv_cell_size));
    int32_t cy =
        static_cast<int32_t>(std::floor(boda.m_posVec.jcap * inv_cell_size));
...
// ── Pure force helper — reads positions from argument, does NOT touch body
// state ──
static void computeAccel(const std::vector<vectorP> &pos,
  ...
  for (int i = 0; i < s - 1; i++) {
    for (int j = i + 1; j < s; j++) {
      ...
      if (bodies[i]->movability)
        a_out[i] += r * (G * bodies[j]->m_Mass / r3);
      if (bodies[j]->movability)
        a_out[j] -= r * (G * bodies[i]->m_Mass / r3);
    }
  }
}
...
void computeAccelBH(const std::vector<vectorP> &pos,
                    std::vector<vectorP> &a_out,
                    const std::vector<std::unique_ptr<Body>> &bodies,
  ...
  for (int i = 0; i < s; i++) {
    if (!bodies[i]->movability)
      continue;
    double ax = 0, ay = 0;
    bh::walk(root, i, pos[i].icap, pos[i].jcap, 0, 0, theta_sq, pos, masses,
             nullptr, ax, ay, nullptr, nullptr);
...
void resolveWithJerkBH(std::vector<std::unique_ptr<Body>> &bodies,
  ...
  for (int i = 0; i < s; i++) {
    ...
    bh::walk(root, i, pos[i].icap, pos[i].jcap, vel[i].icap, vel[i].jcap,
             theta_sq, pos, masses, &vel, ax, ay, &jx, &jy);
    bodies[i]->m_accVec = vectorP(ax, ay);
...
void moveYoshida(std::vector<std::unique_ptr<Body>> &bodies) {
  ...
  for (int step = 0; step < 4; step++) {
    ...
    if (step < 3) {
      // resolveBarnesHut(bodies); // Re-evaluate accelerations at the new
      // positions
...
// Helper function to resolve both Acceleration AND Jerk at current
// positions/velocities Helper function to resolve both Acceleration AND Jerk at
...
void moveHermite(std::vector<std::unique_ptr<Body>> &bodies, double &dt) {
  ...
  for (int i = 0; i < s; i++) {
    ...
    bodies[i]->m_posVec =
        x_pred[i] + snap * (dt4 / 24.0) + crackle * (dt5 / 120.0);
    bodies[i]->m_velVec =
        v_pred[i] + snap * (dt3 / 6.0) + crackle * (dt4 / 24.0);
...
inline int rk45_step_rejections = 0;
inline int rk45_forced_floor_accepts = 0;
...
// Returns: h that was actually used this step → add to your phys_time
// accumulator.
double moveRK45(std::vector<std::unique_ptr<Body>> &bodies) {
  ...
  for (;;) { // retry loop: repeats only on step rejection
    ...
    // Stages k2..k7
    // After st=6: xs = x5 (5th-order solution), vs = v5  [FSAL: A[5] ==
    // b5[0..5]]
    ...
    const bool within_tolerance = (err <= 1.0);
    const bool at_step_floor = (h <= DT_MIN * (1.0 + 1e-12));

    if (within_tolerance || at_step_floor) {
      // ── Accept ──
      // Floor acceptance (INT-003): when h == DT_MIN and err > 1, accuracy is
      // intentionally abandoned to avoid an infinite reject loop — standard
      // practice for adaptive integrators with a minimum step size.
      if (at_step_floor && !within_tolerance)
        ++rk45_forced_floor_accepts;
      else
        rk45_step_rejections = 0;
      ...
      double h_used = h;
      if (at_step_floor && !within_tolerance) {
        // Pin next step at floor after a tolerance-exceeding accept.
        dt = DT_MIN;
      } else {
        double fac =
            (err > 1e-15) ? SAFETY * std::pow(1.0 / err, 0.2) : MAX_FAC;
...
struct BodyInput {
  ...
  double mass, radius;
  int movable; // if its kept bool and a value other than 0 ir 1 is given it
               // goes haywire , atleast for my poor code
};
...
BodyInput getValBod() {
  ...
  } while (temp.movable < 0 ||
           temp.movable > 1); //!= doesnt work because then both need to be
                              //! satisfied which can nevver happen
...
void bodyAdd(std::vector<std::unique_ptr<Body>> &bodies) {
  ...
  auto bod = std::make_unique<Body>(temp.mass, temp.radius, temp.movable,
                                    temp.pos, temp.vel);
  bodies.push_back(std::move(bod));
}
/*std::unique_ptr<Body> bodyExtract(BodyInput temp)
{
        return(std::make_unique<Body>(temp.mass, temp.radius, temp.movable,
vectorP(temp.x, temp.y), vectorP(temp.xv, temp.yv)));
}*/
} // namespace create
...

  {
    ...
    if (operation == 5) {
      ...
      if (moga == 1) {
        ...
        auto figA =
            std::make_unique<Body>(mass_fig8, rad_fig8, true, pos1, vel1);
        auto figB =
            std::make_unique<Body>(mass_fig8, rad_fig8, true, pos2, vel2);
        auto figC =
            std::make_unique<Body>(mass_fig8, rad_fig8, true, pos3, vel3);
      ...
      if (moga == 4) {
        ...
        auto starA = std::make_unique<Body>(mass_binary, radius_binary, true,
                                            posA, velA);
        auto starB = std::make_unique<Body>(mass_binary, radius_binary, true,
                                            posB, velB);
      ...
      if (moga == 7) {
        ...
        auto star =
            std::make_unique<Body>(1000000000000.0f, 1.0f, true, vector);
        auto perf = std::make_unique<Body>(1000000000000.0f, 0.2f, true,
                                           vector2, vector20);
        auto nig = std::make_unique<Body>(1000000000000.0f, 0.2f, true, vector4,
                                          vector40);

        auto star1 =
            std::make_unique<Body>(1000000000000.0f, 1.0f, true, vector1);
        auto perf1 = std::make_unique<Body>(1000000000000.0f, 0.2f, true,
                                            vector21, vector0);
        auto nig1 = std::make_unique<Body>(1000000000000.0f, 0.2f, true,
                                           vector41, vector40);

        auto star11 = std::make_unique<Body>(1000000000000.0f, 1.0f, true,
                                             vector11, vector40);
        auto perf11 =
            std::make_unique<Body>(1000000000000.0f, 0.2f, true, vector211);
        auto nig11 =
            std::make_unique<Body>(1000000000000.0f, 0.2f, true, vector411);
    ...
    if (operation == 4) {
      ...
      for (const auto &b : bodys) // reference is important as otherwise itll
        ...
        do {
          ...
          if (Draw == 1) {
            ...
            for (int i = 0; i < bodys.size(); i++) {
              ...
              DrawSphere(Vector3(temx * RENDER_SCALE, 0, temy * RENDER_SCALE),
                         temr * RENDER_SCALE, RED);
            }
            ...
            DrawText(
                TextFormat("oglinP :  %f i , %f j", oglinP.icap, oglinP.jcap),
                0, 180, 20, BLACK);
            DrawText(TextFormat("linP :  %f i , %f j", linP.icap, linP.jcap), 0,
                     200, 20, BLACK);
            DrawText(TextFormat("oglinP : %f", oglinP.mag()), 0, 220, 20,
                     BLACK);
            DrawText(TextFormat("linPP : %f", linP.mag()), 0, 240, 20, BLACK);
            DrawText(TextFormat("angPdiffn : %f", angPdiffn), 0, 300, 20,
                     BLACK);
            DrawText(TextFormat("linPdiffn :  %f i , %f j", linPdiffn.icap,
                                linPdiffn.jcap),
                     0, 320, 20, BLACK);
            DrawText(TextFormat("linPdiffn : %f", linPdiffn.mag()), 0, 340, 20,
                     BLACK);
            DrawText(TextFormat("velocity :  %f i , %f j",
                                bodys[1]->m_velVec.icap,
                                bodys[1]->m_velVec.jcap),
                     0, 400, 20, BLACK);
            ;
            DrawText(TextFormat("position :  %f i , %f j",
                                bodys[1]->m_posVec.icap,
                                bodys[1]->m_posVec.jcap),
                     0, 420, 20, BLACK);
            ;
            DrawText(TextFormat("velocity :  %f i , %f j",
                                bodys[1]->m_forVec.icap,
                                bodys[1]->m_forVec.jcap),
                     0, 440, 20, BLACK);

            DrawText(TextFormat("dt : %f", dt), 1000, 20, 20, RED);
            DrawText(TextFormat("velocity : %f", bodys[1]->m_velVec.mag()), 0,
                     500, 20, BLACK);
            DrawText(TextFormat("position : %f", bodys[1]->m_posVec.mag()), 0,
                     520, 20, BLACK);
          ...
          while (!WindowShouldClose() && !quit && !bodys.empty()) {
            ...
            std::cout << "[t = " << frame * dt << "s | frame" << frame
                      << "] Steps ? ( Enter = 1 , q = quit) : ";
            std::string line;
            ...
            if (line == "q" || line == "Q") {
              ...
              try {
                hmframe = std::max(1, std::stoi(line));
              } catch (...) {
                hmframe = 1;
              }
            }
            ...
            for (int s = 0;
              ...
              /*auto ks_pairs =
              ks_regularization::extractCloseEncounters(bodys);
              ...
              if (Draw == 1) {
                ...
                for (int i = 0; i < bodys.size(); i++) {
                  ...
                  float temr = temu->m_radius;
                  DrawSphere(
                      Vector3(temx * RENDER_SCALE, 0, temy * RENDER_SCALE),
                      temr * RENDER_SCALE, RED);
                }
                ...
                DrawText(TextFormat("ogangP : %f", ogangP), 0, 140, 20, BLACK);
                DrawText(TextFormat("angP : %f", angP), 0, 160, 20, BLACK);
                DrawText(TextFormat("oglinP :  %f i , %f j", oglinP.icap,
                                    oglinP.jcap),
                         0, 180, 20, BLACK);
                DrawText(
                    TextFormat("linP :  %f i , %f j", linP.icap, linP.jcap), 0,
                    200, 20, BLACK);
                DrawText(TextFormat("oglinP : %f", oglinP.mag()), 0, 220, 20,
                         BLACK);
                DrawText(TextFormat("linPP : %f", linP.mag()), 0, 240, 20,
                         BLACK);
                DrawText(TextFormat("angPdiffn : %f", angPdiffn), 0, 300, 20,
                         BLACK);
                DrawText(TextFormat("linPdiffn :  %f i , %f j", linPdiffn.icap,
                                    linPdiffn.jcap),
                         0, 320, 20, BLACK);
                DrawText(TextFormat("linPdiffn : %f", linPdiffn.mag()), 0, 340,
                         20, BLACK);
                ...
                DrawText(TextFormat("velocity : %f", bodys[1]->m_velVec.mag()),
                         0, 500, 20, BLACK);
                DrawText(TextFormat("position : %f", bodys[1]->m_posVec.mag()),
                         0, 520, 20, BLACK);

                DrawText(TextFormat("dt : %f", dt), 1000, 20, 20, RED);
                DrawText(TextFormat("phys_time : %f", phys_time), 1000, 40, 20,
                         RED);
              ...
              // t0).count() ; double dt_ms = dt * 1000000.0f; LOG("work_ms = "
              // << work_ms << " dt_ms = " << dt_ms << "\n"); this is for the
              // work ms , comment it out auto t0 = clock::now(); //iteration
              ...
              /*if (frame == 1000)
              {
                      ...
                      auto work_us =
              std::chrono::duration_cast<std::chrono::microseconds>(t1 -
              t0).count() ; LOG("work_ms = " << work_us << " dt_us = " << dur *
              1000000 << "\n");
              }*/
            }
          }
      ...
      if (stat == 1 || stat == 0) {
        do {
          ...
          while (!WindowShouldClose()) {
            ...
            auto dt_duration = std::chrono::duration_cast<clock::duration>(

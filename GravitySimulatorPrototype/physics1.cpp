...
namespace fmm {
                        ...
                        const std::vector<std::unique_ptr<Body>>& bodies)
    {
        ...
        for (int u_leaf : leaves) {
            ...
            // ── Near-field P2P (neighboring leaves) ──────────────────────────────
            // KERNEL: 2D log-law gravity  F = G m r / r²  (force ∝ 1/r).
            //
            // WHY: the complex multipole operators (P2M→M2M→M2L→L2L→L2P) encode
            // the kernel  1/(z_s − z),  whose real/imag parts give:
            //   F_x = Gm(x_s−x)/r²,  F_y = Gm(y_s−y)/r²  — i.e. |F| ∝ 1/r.
            // The original P2P here used  inv_r3 (3D Newton, |F| ∝ 1/r²), which
            // created a hard discontinuity at the near/far boundary and mixed two
            // incompatible physics laws — the primary cause of the 5-order energy
            // drift observed in RK45 over 10 000 frames.
            //
            // NOTE FOR THE USER: the entire FMM (far-field AND this near-field)
            // now consistently implements 2D log-law gravity.  This is DIFFERENT
            // from the 3D Newtonian law used by computeAccel / BH / Verlet /
            // Yoshida / Hermite and eos().  For 3D Newtonian gravity use
            // computeAccelBH / resolveBarnesHut instead.  The BH is already
            // correct and internally consistent with the rest of the integrators.
            for (int v_leaf : leaves) {
                ...
                for (int i : u_nd.bodies) {
                    ...
                    for (int j : tree[v_leaf].bodies) {
                        ...
                        vectorP r   = pos[j] - pos[i];   // r points i → j
                        double  r2  = r.magSq();
                        if (r2 < 1e-20) continue;         // guard exact overlap
                        double  inv_r2 = 1.0 / r2;       // 1/r²: gives |F| ∝ 1/r

                        acc_out[i] += r * (G * masses[j] * inv_r2);

                        if (jerk_out && vel_ptr) {
                            // Jerk for F = G m r / r²:
                            //   da/dt = G m [ v_rel/r² − 2(v_rel·r)r / r⁴ ]
                            vectorP v       = (*vel_ptr)[j] - (*vel_ptr)[i];
                            double  v_dot_r = v.icap * r.icap + v.jcap * r.jcap;
                            double  inv_r4  = inv_r2 * inv_r2;
                            (*jerk_out)[i] += (v * inv_r2 - r * (2.0 * v_dot_r * inv_r4))
                                              * (G * masses[j]);
                        }
                    }
                }

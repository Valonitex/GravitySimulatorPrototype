...
public:
  ...
  vectorP operator*(double t) { return (vectorP(icap * t, jcap * t)); }
  ...
  for (int i = 0; i < (nBodies - 1); i++) {
    ...
    for (int j = i + 1; j < nBodies; j++) {
      ...
      if (disp < mindisp) {
        ...
        if (clusInA !=
            clusInB /* && clusInA != 0 && clusInB != 0*/) // could potentially
                                                          // remove clusInB != 0
                                                          // , as it doesnt
                                                          // matter in the case
                                                          // A is not zero and
...
void moveHermite(std::vector<std::unique_ptr<Body>> &bodies, double &dt) {
  ...
  dt =
      std::max(dt_candidate, 1e-7); // floor prevents dt → 0 on singular configs
}
...
// ─────────────────────────────────────────────────────────────────────────
void computeAccRK(const std::vector<std::unique_ptr<Body>> &bodies,
...
// ─────────────────────────────────────────────────────────────────────────
void moveRK45(std::vector<std::unique_ptr<Body>> &bodies, double &dt,
              double tol = 1e-6, double dt_max = 1.0 / 30.0) {
  const int n = static_cast<int>(bodies.size());
  if (n == 0)
    return;
  ...
  std::vector<vectorP> k1p(n), k1a(n), k2p(n), k2a(n), k3p(n), k3a(n), k4p(n),
      k4a(n), k5p(n), k5a(n), k6p(n), k6a(n), k7p(n), k7a(n), sp(n),
      sv(n); // scratch stage positions / velocities
  ...
  while (!accepted) {
    ...
    for (int i = 0; i < n; i++) {
      sp[i] = x0[i] + k1p[i] * (a61 * dt) + k2p[i] * (a62 * dt) +
              k3p[i] * (a63 * dt) + k4p[i] * (a64 * dt) + k5p[i] * (a65 * dt);
      sv[i] = v0[i] + k1a[i] * (a61 * dt) + k2a[i] * (a62 * dt) +
              k3a[i] * (a63 * dt) + k4a[i] * (a64 * dt) + k5a[i] * (a65 * dt);
...
static inline double vMag(const vectorP &a) { return std::sqrt(vMagSq(a)); }
...
static inline vectorP lc_uToCart(const vectorP &u) {
  return vectorP(u.icap * u.icap - u.jcap * u.jcap, 2.0 * u.icap * u.jcap);
}
...
static inline double lc_bindingEnergy(const vectorP &Xdot, double r,
                                      double Mpair) {
...
static ChainState
lc_buildChain(const std::vector<std::unique_ptr<Body>> &bodies) {
  ChainState chain;
  ...
  for (int k = 0; k < nLinks; k++) {
    ...
    if (bodies[si]->movability)
      Mpair += bodies[sj]->m_Mass;
    if (bodies[sj]->movability)
      Mpair += bodies[si]->m_Mass;
...
static void
lc_chainToPositions(const ChainState &chain,
  ...
  if (N == 1) {
    pos[chain.sigma[0]] =
        (pinned_idx != -1) ? bodies[chain.sigma[0]]->m_posVec : chain.R;
    return;
  }
...
static void
lc_chainToVelocities(const ChainState &chain,
                     const std::vector<std::unique_ptr<Body>> &bodies,
  ...
  if (N == 1) {
    vel[chain.sigma[0]] =
        (pinned_idx != -1) ? bodies[chain.sigma[0]]->m_velVec : chain.V_cm;
    return;
  }
...
static vectorP lc_perturbation(const ChainState &chain,
                               const std::vector<std::unique_ptr<Body>> &bodies,
                               const std::vector<vectorP> &positions, int k) {
  int si = chain.sigma[k];
  int sj = chain.sigma[k + 1];
  int N = chain.N;
...
static ChainState lc_addScaled(const ChainState &state, const ChainDerivs &f,
                               double h) {
...
static void lc_drift(ChainState &chain, double ds) {
  ...
  for (int k = 0; k < nLinks; k++) {
    ...
    chain.links[k].u =
        vAdd(chain.links[k].u, vScale(chain.links[k].w, alpha * ds));
  }
...
static void lc_kick(ChainState &chain,
  ...
  for (int k = 0; k < nLinks; k++) {
    ...
    vectorP dw = vScale(vAdd(vScale(chain.links[k].u, chain.links[k].h * 0.5),
                             vScale(LT_Pk, rk * 0.5)),
                        alpha);
...
void moveVerletLC(std::vector<std::unique_ptr<Body>> &bodies, double &dt) {
  ...
  if (N == 1) {
    if (bodies[0]->movability)
      bodies[0]->m_posVec =
          vAdd(bodies[0]->m_posVec, vScale(bodies[0]->m_velVec, dt));
    resolve(bodies);
    return;
  }
...
double moveHermiteLC(std::vector<std::unique_ptr<Body>> &bodies, double &dt) {
  ...
  for (int k = 0; k < nLinks; k++) {
    ...
    state_corr.links[k].u =
        vAdd(state_corr.links[k].u, vScale(vAdd(f0.du[k], f1.du[k]), ds * 0.5));
    state_corr.links[k].w =
        vAdd(state_corr.links[k].w, vScale(vAdd(f0.dw[k], f1.dw[k]), ds * 0.5));
    state_corr.links[k].h += (f0.dh[k] + f1.dh[k]) * ds * 0.5;

    // Hermite correction: Δs²·(j₀−j₁)/12
    state_corr.links[k].u =
        vAdd(state_corr.links[k].u, vScale(vSub(j0.du[k], j1.du[k]), ds2_12));
    state_corr.links[k].w =
        vAdd(state_corr.links[k].w, vScale(vSub(j0.dw[k], j1.dw[k]), ds2_12));
    state_corr.links[k].h += (j0.dh[k] - j1.dh[k]) * ds2_12;
  }
  state_corr.R = vAdd(state_corr.R, vScale(vAdd(f0.dR, f1.dR), ds * 0.5));
  state_corr.R = vAdd(state_corr.R, vScale(vSub(j0.dR, j1.dR), ds2_12));
  ...
  for (int k = 0; k < nLinks; k++) {
    ...
    vectorP snap_k = vScale(
        vSub(vScale(vSub(f0.dw[k], f1.dw[k]), -6.0),
             vScale(vAdd(vScale(j0.dw[k], 4.0), vScale(j1.dw[k], 2.0)), ds)),
        1.0 / (ds * ds));
    vectorP crackle_k = vScale(vAdd(vScale(vSub(f0.dw[k], f1.dw[k]), 12.0),
                                    vScale(vAdd(j0.dw[k], j1.dw[k]), 6.0 * ds)),
                               1.0 / (ds * ds * ds));
...
double moveRK45LC(std::vector<std::unique_ptr<Body>> &bodies, double &dt,
                  double tol /*= 1e-6*/, double dt_max /*= 1.0 / 30.0*/) {
  int N = static_cast<int>(bodies.size());
  if (N == 0)
    return dt;
  ...
  while (!accepted) {
    ...
    for (int k = 0; k < nLinks; k++) {
      ...
      double eh = (k1.dh[k] * e1 + k3.dh[k] * e3 + k4.dh[k] * e4 +
                   k5.dh[k] * e5 + k6.dh[k] * e6 + k7.dh[k] * e7) *
                  ds;
...
// ── HYBRID INTEGRATOR WRAPPERS ───────────────────────────────────────────

void moveVerletHybridLC(std::vector<std::unique_ptr<Body>> &bodies,
                        double etaSq) {
  double r2_min = 1e30;
  int n = static_cast<int>(bodies.size());
  for (int i = 0; i < n; i++) {
    for (int j = i + 1; j < n; j++) {
      double d2 = (bodies[i]->m_posVec - bodies[j]->m_posVec).magSq();
      if (d2 < r2_min)
        r2_min = d2;
    }
  }
  ...
  if (r2_min < etaSq) {
    ...
    moveVerletLC(bodies, current_dt);
    ::dt = current_dt;
  } else {
    moveVerlet(bodies);
  }
...
void moveYoshidaHybridLC(std::vector<std::unique_ptr<Body>> &bodies,
                         double etaSq) {
  double r2_min = 1e30;
  int n = static_cast<int>(bodies.size());
  for (int i = 0; i < n; i++) {
  ...
  if (r2_min < etaSq) {
    double current_dt = ::dt;
    moveYoshidaLC(bodies, current_dt);
    ::dt = current_dt;
  } else {
    moveYoshida(bodies);
  }
...
int main() {
...

  {
    ...
    if (operation == 5) {
      ...
      vector2,vector20); auto nig = std::make_unique<Body>(1000000000000.0f,
      0.2f, true, vector4,vector40);


      auto star1 = std::make_unique<Body>(1000000000000.0f, 1.0f, true,
      vector1); auto perf1 = std::make_unique<Body>(1000000000000.0f, 0.2f,
      true, vector21,vector0); auto nig1 =
      std::make_unique<Body>(1000000000000.0f, 0.2f, true, vector41, vector40);

      auto star11 = std::make_unique<Body>(1000000000000.0f, 1.0f, true,
      vector11, vector40); auto perf11 =
      std::make_unique<Body>(1000000000000.0f, 0.2f, true, vector211); auto
      nig11 = std::make_unique<Body>(1000000000000.0f, 0.2f, true, vector411);
    */
          ...
          while (!WindowShouldClose() && !quit && !bodys.empty()) {
            ...
            for (int s = 0;
              ...
              if (Draw == 1) {
                ...
                DrawText(TextFormat("phys_time : %f", phys_time), 1000, 60, 20,
                         RED);
